...
if ( !class_exists( 'CmindsLicensingAPI' ) ) {
	...
	class CmindsLicensingAPI {
		...
		private $cmindsPackage						 = null;
		private $url								 = null;
		private $shortslug							 = null;
		private $slug								 = null;
		private $name								 = null;
		private $plugin								 = null;
		...
		/**
		 *
		 * @param CmindsPackage $cmindsPackage
		 */
		public function __construct( $cmindsPackage ) {

			$this->cmindsPackage = $cmindsPackage;

			$this->url		 = get_bloginfo( 'wpurl' );
			$this->shortslug = $cmindsPackage->getOption( 'plugin-shortslug' );
			$this->slug		 = basename( $cmindsPackage->getOption( 'plugin-file' ), '.php' );
			$this->plugin	 = $this->name		 = $cmindsPackage->getOption( 'plugin-basename' );
			$this->version	 = $cmindsPackage->getOption( 'plugin-version' );

			$this->pluginMenu			 = $cmindsPackage->getOption( 'plugin-menu-item' );
			...
			$this->pluginFile	 = $cmindsPackage->getOption( 'plugin-file' );
			$this->changelogUrl	 = $cmindsPackage->getOption( 'plugin-changelog-url' );

			$this->pluginName	 = $cmindsPackage->getOption( 'plugin-name' );
			$this->pluginSlug	 = $cmindsPackage->getOption( 'plugin-slug' ) ? $cmindsPackage->getOption( 'plugin-slug' ) : self::camelCaseToHypenSeparated( $this->pluginName );
			...
			$this->itemName		 = $cmindsPackage->getOption( 'plugin-license-name' );

			$this->validItemNames	 = array( $this->itemName );
			$licensingAliases		 = $cmindsPackage->getOption( 'plugin-licensing-aliases' );
...
//			add_action( 'admin_notices', array( $this, 'showMessage' ) );
				...
				<form method="post" action="options.php">
					...
					<table class="form-table">
						<tbody>
							 ...
							 */
							do_action( 'cminds-' . $this->shortslug . '-license-page' );
							?>
							<tr valign="top">
		...
		public function update_page() {
			...
			<div>
				...
				<table class="cminds_update_table">
					<tr>
						<th>Your version</th>
				 ...
				 * Call the action in the add-ons
				 */
				do_action( 'cminds-' . $this->shortslug . '-update-page' );
				?>
			</div>
			<?php
			$content .= ob_get_clean();
