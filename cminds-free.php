...
//			add_action( 'cminds-' . $aboutPageKey . '-content-5', array( $this, 'displayUpgradeToProTab' ) );
			...
			include_once "cminds-licensing-api.php";
...
//				add_action( 'cminds-' . $licensePageKey . '-content-30', array( $this, 'displayUpgradeTutorialTab' ) );
		...
		public function updateMenu() {
			...
			$tag		 = 'cminds-' . $this->getOption( 'plugin-shortslug' ) . '-license-page';
...
//						'30' => 'Upgrade Tutorial',
		...
		public function showRegistration() {
			...
			$optionName			 = 'cminds-' . $this->getOption( 'plugin-shortslug' ) . '-registration-hidden';
			...
			$optionName	 = 'cminds-' . $this->getOption( 'plugin-shortslug' ) . '-registered';
			...
			if ( !empty( $post ) && !empty( $post[ 'cminds_nonce' ] ) ) {
				...
				if ( $nonceCheck ) {
					...
					$href		 = 'https://demo.cminds.com/wp-admin/admin-ajax.php?action=add_user&cminds_json_api=add_user';
					...
					<div class="cminds_registration_wrapper">
						<div class="cminds_registration">
							<form method="post" action="">
								...
								echo $this->getRegistrationFields();
								?>
...
$cmindsPluginPackage[ 'cmtt' ] = new CmindsFreePackage( $cminds_plugin_config );
